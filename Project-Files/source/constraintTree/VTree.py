...
class _VTree(angr.Analysis):
    ...
    def _checkForFixIndexAccess(self,current_stack,m_addr,wr_addr,state):
        ...
        if index and index >= malloc_size:
            message='There Is Buffer Overflow In Block {} with Constant Write '.format(wr_addr)
        ...
        if isinstance(out_str,str):
            ...
            if dst_size:
                if len(out_str) > dst_size:
                    ...
                    self._vulReports.append('There Is Buffer Overflow In Block {} with Target Function [sprintf] '.format(vex.addr))
    ...
    def _getVulConstraintsForMemset(self,vex,props,rhs,history_blocks,wchar=False):
        ...
        if dst_size:
            message='There Is Buffer Overflow In Block {} with Target Function [memset] '.format(vex.addr)
    ...
    def _getVulConstraintsForMemcpy(self,vex,props,rhs,history_blocks,wchar=False):
        func_name,limit=rhs
        message='There Is Buffer Overflow In Block {} with Target Function [{}]'.format(vex.addr,props[0])
    ...
    def _getVulConstraintsForStrcpy(self,vex,props,rhs,history_blocks):
        message='There Is Buffer Overflow In Block {} with Target Function [strcpy]'.format(vex.addr)
        if isinstance(rhs,str):
            ...
            if src_size and dst_size:
                ...
                const.append(claripy.UGT(rhs ,dst_size))# len(src) > len(dst) 
    ...
    def _getVulConstraintsForStrcat(self,vex,props,rhs,history_blocks):
        ...
        message='There Is Buffer Overflow In Block {} with Target Function [strcat]'.format(vex.addr)
